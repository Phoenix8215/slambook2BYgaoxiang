...
Mat CannyEdgeDetection(cv::Mat src, int kSize, double hightThres, double lowThres) {
    ...
    for (int i = gaussCenter; i < row - gaussCenter; i++) {
        for (int j = gaussCenter; j < col - gaussCenter; j++) {
            ...
            filterImg.at<double>(i, j) = cv::sum(guassKernel.mul(src(rect)))[0];
        }
    }
    ...
    for (int i = 1; i < row - 1; i++) {
        for (int j = 1; j < col - 1; j++) {
            ...
            // 梯度和角度
            gradX.at<double>(i, j) += cv::sum(rectImg.mul(Sx))[0];

            gradY.at<double>(i, j) += cv::sum(rectImg.mul(Sy))[0];
    ...
    // 双阈值确定
    cv::Mat caculateValue = cv::Mat::zeros(row, col, CV_64FC1);  
    // grad变成一维
